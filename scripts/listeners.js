...
export const panListener = () => {
  ...
  canvas.addEventListener("pointermove", (e) => {
    ...
    let renderW, renderH, canvasW, canvasH;
    if (shape === "original") {
      canvasW = img.width;
      canvasH = img.height;
      renderW = img.width * z;
    ...
    const maxPanX = Math.max(canvasW, renderW) / 2;
    const maxPanY = Math.max(canvasH, renderH) / 2;

    let newX = initialX + (canvasDx / maxPanX) * 100;
    let newY = initialY + (canvasDy / maxPanY) * 100;
  ...
  canvas.addEventListener(
    ...
    (e) => {
      ...
      newZ = Math.max(0.1, Math.min(5, newZ));
